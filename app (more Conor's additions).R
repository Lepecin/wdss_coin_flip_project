...
server<-function(input,output){
  ...
  library(shiny)
  {
    ...
    # Distribution of last equalisation 
    problastequalisation<-function(n,p){
      out<-numeric((n+1))
      zeros<-numeric((n+1))
      i<-3
      zeros[1]<-1
      while(i<=n+1){
        ...
        k<-as.numeric(0)
        while(j<=i){
          k=k+(choose(j,j/2)*(p^(j/2))*((1-p)^(j/2))*zeros[i-j])
          j=j+2
        }
        zeros[i]=1-k
        i=i+2
      ...
      a<-1
      while(a<=n+1){
        out[a]=choose(a-1,(a-1)/2)*(p^((a-1)/2))*((1-p)^((a-1)/2))*zeros[n+1-(a-1)]
        a=a+2
      }
      return(out) 
    }
    
    lastequalisation<-function(x){
      i<-2
      obs<-0
      while(i<=length(x)){
        k<-0
    ...
    lastequalisationpval<-function(n,p,k){
      probarray<-problastequalisation(n,p)
      obsprob<-probarray[k+1]
      pval<-0
      for(i in seq(from = 1, to = n+1, by = 2)){
        curprob<-probarray[i]
        if(curprob<=obsprob){pval=pval+curprob}
      }
      return(pval)
    }
    ...
    pvaldata<-function(y,p){
      d<-data.frame(
        ...
        "Pvalues" = c(
          ...
          multitest(length(y),p,consecutivetosses(y)),
          lastequalisationpval(length(y),p,lastequalisation(y))
        )
      )
      return(d)
    }
  ...
  output$sampledata<-renderText(
    if(nchar(input$coinstring)!=as.numeric(input$stringlength)){
      ...
      paste(
        ...
        as.character(consecutivetosses(y)[1]),
        as.character(consecutivetosses(y)[2]),
        as.character(consecutivetosses(y)[3]),
        as.character(consecutivetosses(y)[4]),
        "// Last equalisation after flip:",
        as.character(lastequalisation(y))
